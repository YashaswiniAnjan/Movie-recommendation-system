...
def get_als_recommendations(user_id, n=5):
    user_df = ratings_spark.filter(
        col("user_id") == user_id
    ).select("user_id").distinct()

    recommendations = als_model.recommendForUserSubset(user_df, n)
    ...
    recommended_ids = [
        r["movie_id"]
        for r in recommendations.select("recommendations").collect()[0]["recommendations"]
    ]
